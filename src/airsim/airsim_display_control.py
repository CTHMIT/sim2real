...
class AirSimClient:
    ...
    def get_image(self, client_instance=None):
        ...
        try:
            ...
            # Update camera gimbal if control state has values
            if hasattr(self, "state") and (
                self.state.pitch != 0 or self.state.roll != 0 or self.state.yaw != 0
            ):
                quat = airsim.euler_to_quaternion(
                    self.state.roll * np.pi / 180,
                    self.state.pitch * np.pi / 180,
                    self.state.yaw * np.pi / 180,
                )
    ...
    def process_and_display(self):
        ...
        if key == ord("q"):
            ...
            self.config.display.output_width = min(
                self.config.display.output_width + 80, 1920
            )
            self.config.display.output_height = min(
                self.config.display.output_height + 60, 1080
            )
            cv2.resizeWindow(
            ...
            LOGGER.info(
                f"Resolution increased to: {self.config.display.output_width}x{self.config.display.output_height}"
            )
        elif key == ord("-"):
            self.config.display.output_width = max(
                self.config.display.output_width - 80, 160
            )
            self.config.display.output_height = max(
                self.config.display.output_height - 60, 120
            )
            cv2.resizeWindow(
                self.config.display.window_name,
                self.config.display.output_width,
            ...
            LOGGER.info(
                f"Resolution decreased to: {self.config.display.output_width}x{self.config.display.output_height}"
            )

        return not self.state.exit_flag
...
def main():
    ...
    try:
        ...
        try:
            ...
            # Set window name and resize it to match the image
            cv2.namedWindow(config.display.window_name, cv2.WINDOW_NORMAL)
            cv2.resizeWindow(
                config.display.window_name, image_info["width"], image_info["height"]
            )
        ...
        while True:
            ...
            if elapsed >= args.stats_interval and args.stats_interval > 0:
                # Get performance statistics
